...
class FastBertNormalizer(module.Module):
  ...
  def normalize(self, input):  # pylint: disable=redefined-builtin
    ...
    normalized_texts, _ = self.normalize_with_offsets_map(input)
    return normalized_texts

  def normalize_with_offsets_map(self, input):  # pylint: disable=redefined-builtin
    ...
    >>> texts = ["They're", "the", "Greatest", "\xC0bc"]
    >>> normalizer = FastBertNormalizer(lower_case_nfd_strip_accents=True)
    >>> normalized_text, offsets_mapping = (
    ...   normalizer.normalize_with_offsets_map(texts))
    >>> normalized_text
    ...
    Returns:
      A tuple `(texts, offsets_map)` where:

      texts: is a `Tensor` or `RaggedTensor`.
      offsets_map: is a `RaggedTensor` of the byte offsets from the output
        to the input.
    """
    name = None
    with ops.name_scope(name, 'FastBertNormalize', [input, self._model]):
      ...
      if rank == 0:
        normalized_texts, offsets_map = self.normalize_with_offsets_map(
            array_ops.stack([input]))
        return normalized_texts.values, offsets_map.values
        ...
        normalized_texts, offsets_map = self.normalize_with_offsets_map(
            input.flat_values)
        offsets_map = offsets_map.with_row_splits_dtype(input.row_splits.dtype)
        return (input.with_flat_values(normalized_texts),
                input.with_flat_values(offsets_map))

      # TODO(xysong): Optimize below by calling different overload kernels.
      normalized_texts, offsets_map, row_splits = (
          gen_fast_bert_normalizer.fast_bert_normalize(
              input_values=input, fast_bert_normalizer_model=self._model))

      offsets_map = RaggedTensor.from_row_splits(
          offsets_map, row_splits, validate=False)

      return normalized_texts, offsets_map
