...
class FastBertNormalizeOp
 ...
 public:
  ...
  Returns:
    ...
    * output_offset_mappings: 1D tensor containing the offset mapping from the
      normalized text to the original text. A 2D RaggedTensor can be constructed
      from this and output_row_splits.
    * output_row_splits: 1D int tensor with the row splits that allow us to
      build RaggedTensors from output_offset_mappings.
)doc";
...
absl::Status FastBertNormalizeOp<Rt>::Invoke(InvokeContext* context) {
  ...
  std::vector<std::string> normalized_strings;
  SH_ASSIGN_OR_RETURN(
      auto output_values,
  ...
  for (int i = 0; i < values_vec.Dim(0); ++i) {
    ...
    text_normalizer->template NormalizeText</*kGetOffsets=*/true>(
